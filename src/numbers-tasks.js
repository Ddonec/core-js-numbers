...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengh1 = Math.sqrt(x1 * x1 + y1 * y1);
  const lengh2 = Math.sqrt(x2 * x2 + y2 * y2);
  const cos = scalar / (lengh1 * lengh2);
...
function getSumOfNumbers(x1, x2, x3) {
  const summ = x1 + x2 + x3;
  return Number.parseFloat(summ.toFixed(1));
}
...
function getRandomInteger(min, max) {
  ...
  const random = Math.floor(Math.random() * diap);
  return random + min;
}
